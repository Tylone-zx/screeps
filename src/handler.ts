...
import {
  role,
  roleAttackerTaskList,
  roleHarvesterTaskList,
  roleBuilderTaskList,
  roleMinerTaskList,
  roleMiscerTaskList,
  roleTransferrerTaskList,
  roleUpgraderTaskList
} from "role";
...
export const handler = () => {
  ...
  for (const [name, creep] of Object.entries(Game.creeps)) {
    if (name.startsWith(ROLE_HAVESTER)) {
      role.run(creep, roleMinerTaskList);
    }
    if (name.startsWith(ROLE_UPGRADER)) {
      role.run(creep, roleUpgraderTaskList);
    }
    if (name.startsWith(ROLE_BUILDER)) {
      role.run(creep, roleBuilderTaskList);
    }
    if (name.startsWith(ROLE_MISCER)) {
      role.run(creep, roleMiscerTaskList);
    }
    if (name.startsWith(ROLE_MOVER)) {
      role.run(creep, roleTransferrerTaskList);
    }
    if (name.startsWith(ROLE_ATTACKER)) {
      role.run(creep, roleAttackerTaskList);
    }
  }
